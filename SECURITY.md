...
- 推荐：个人电脑、本机应用、可信内网，或只发布 Web 静态模式。
...
- Node 运行时主库是 `data/starvault.db`。旧 `data/store.json` 只作为首次迁移来源和人工备份，仍可能包含历史密钥与用户数据。
...
- 扫描、AI 分析和方案生成由有界并发队列执行，最大并发为 2，持久化任务最多重试 3 次。
...
公开仓库前还应运行密钥模式扫描，并人工确认 `.env`、`data/`、`output/` 和任何用户数据没有进入 Git。

## 漏洞报告

请通过邮件私下报告安全问题：946106011@qq.com。报告中不要附带真实 Token、Key 或其他用户的敏感数据。
