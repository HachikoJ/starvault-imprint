...
const { legacyProfileKey, mergeProfileMatches, profileEvidence } = require("../../public/domain-core");
...
function sharedPlanAnchorTerms(plan = {}, queries = []) {
  const items = Array.isArray(queries) ? queries : [];
  if (!items.length) return [];
  return planFilterTerms(plan).filter((term) =>
    items.every((item) => queryContainsPlanTerm(typeof item === "string" ? item : item?.query || item?.q || "", term))
  );
}
...
function repositoryMatchesPlanProfile(repo = {}, profile = {}) {
  ...
  const terms = Array.isArray(profile.requiredPlanTerms) && profile.requiredPlanTerms.length
    ? profile.requiredPlanTerms.filter(Boolean)
    : Array.isArray(profile.planTerms)
      ? profile.planTerms.filter(Boolean)
      : [];
...
function buildObservationPlanProfiles(plan, defaults) {
  ...
  const requiredPlanTerms = sharedPlanAnchorTerms(plan, rawQueries);
  const seen = new Set();
